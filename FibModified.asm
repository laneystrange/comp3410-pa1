...
size: .word 0 #initialize the size
welcome: .asciiz "Please enter a number from 1 - 9:  " # displays a welcome message for user input
error: .asciiz "YOU ENTERED AN INVALID NUMBER! RUN THE PROGRAM AGAIN." #invalid number string notification
...
###############################################
	.text
	try: # restarts if the incorrect value is input
	      # print out the welcome message and ask for user input
	      li $v0, 4
              la $a0, welcome
              syscall
              # get the user input
	      li $v0, 5
              syscall
              sw $v0, size # save the user input into size
              
              la   $s5, size        # load address of size variable into $s5 	
              lw   $s5, 0($s5)      # load array size from its address in the register
              
              li $s7, 9 # this register is set the highest possible value size is allowed to be
              bgt $s5, $s7, try # checks if size is greater than 9
              li $s7, 1 # this register is set to the lowest possible value size is allowed to be
	      blt $s5, $s7, try # checks if size is less than 1
